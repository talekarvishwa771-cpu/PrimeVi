...
function renderVideoShowcasePublic(){
  const wrap = document.getElementById("videoShowcaseGrid");
  if(!wrap) return;
  wrap.innerHTML = videoShowcase.map(v => `
    <div class="vcard">
      <div class="vthumb ${escapeHtml(v.gradient)}"><div class="vplay" role="button" aria-label="Play preview"></div><span class="vtime mono">${escapeHtml(v.duration)}</span></div>
...
function addPortfolioRow(){
  ...
  videoShowcase.push({ title: "", tag: "", duration: "00:00", gradient: "grad-1" });
